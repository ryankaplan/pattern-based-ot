...
class OTClient {
  ...
  // Always keeps track of the last total ordering id that we've seen from the
  // server. That might be the total ordering id of the last remote op that
  // has been sent to us, or the total ordering id of the last ack of a local
  // op that we sent to the server.
  ...
  public handleRemoteOp(op:Operation) {
    ...
    if (op.timestamp().siteId() === this._siteId) {
      ...
      // _lastRemoveTotalOrderingId is the total ordering id of the last operation that
      // we've seen from the server.
